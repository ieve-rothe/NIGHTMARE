...
module Nightmare::Tools
  ...
  class Shell
    ...
    property approval_handler : Proc(String, Array(String), Bool, Tuple(ApprovalOutcome, String?))?
    property active_side_effects : Array(String)?

    def initialize(
      @guard : Guard,
      @allowlist : Allowlist = Allowlist.new,
      @approval_handler : Proc(String, Array(String), Bool, Tuple(ApprovalOutcome, String?))? = nil
    )
    end
      ...
      loop do
        ...
        unless is_auto
          if handler = @approval_handler
            outcome, edit_value = handler.call(cmd_to_run, argv, has_metachar)
            case outcome
            when ApprovalOutcome::No
              return "[Execution rejected by user]"
            when ApprovalOutcome::AllSession
              @allowlist.allow_session_exact(argv) unless has_metachar
            when ApprovalOutcome::PrefixSession
              @allowlist.allow_session_prefix(argv) unless has_metachar
            when ApprovalOutcome::PrefixPersist
              @allowlist.allow_persist_prefix(argv) unless has_metachar
            when ApprovalOutcome::Edit
              if edited = edit_value
                cmd_to_run = edited
                next # re-evaluate loop with edited command
              else
                return "[Execution rejected by user]"
              end
        ...
        return execute_process_group(argv, timeout_seconds)
      end
    end

    private def execute_process_group(argv : Array(String), requested_timeout : Int32?) : String
      ...
      cmd_args = has_setsid ? (["-w"] + argv) : argv[1..]
      ...
      when timeout(timeout)
        ...
        final_status = exit_status_channel.receive rescue nil
      end
    ...
    private def terminate_process_group(pgid : Int64) : Nil
